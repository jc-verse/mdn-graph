...
function urlToMacro(url: string) {
  if (/Web\/API\/\w+$/.test(url)) {
    ...
    return [
      new RegExp(
        String.raw`\{\{\s*domxref\(\s*["']${target}["']\s*\)\s*\}\}`,
        "ig",
      ),
      () => `\`${target}\``,
    ];
    ...
    return [
      new RegExp(
        String.raw`\{\{\s*domxref\(\s*(?:["'](?<nameAndUrl>${target}\.${member}(?:\(\))?)["']|["']${target}.${member}(?:\(\))?["'],\s*["'](?<name>[^"']+)["']|)\s*\)\s*\}\}`,
        "ig",
      ),
      (_, nameAndUrl, name) => `\`${nameAndUrl ?? name}\``,
    ];
  } else if (/Web\/HTTP\/Headers\/\w+$/.test(url)) {
    const target = /Web\/HTTP\/Headers\/(\w+)$/.exec(url)![1];
    return [
      new RegExp(
        String.raw`\{\{\s*httpheader\(\s*["']${target}["']\s*\)\s*\}\}`,
        "ig",
      ),
      () => `\`${target}\``,
    ];
    ...
    return [
      new RegExp(
        String.raw`\{\{\s*htmlelement\(\s*["']${target}["']\s*\)\s*\}\}`,
        "ig",
      ),
      () => `\`<${target}>\``,
    ];
    ...
    return [
      new RegExp(
        String.raw`\{\{\s*cssxref\(\s*["']${target}["']\s*\)\s*\}\}`,
        "ig",
      ),
      () => `\`${target}\``,
    ];
...
for (const [location, target] of selfLinks
  .split("\n")
  .map((x) => x.split("\t"))) {
