...
- It is not aware of Markdown source, or how Markdown source correlates to the HTML output. The most important feature missing with this restriction is the ability to check macros. Rather, we only make sure that the rendered output is correct, but not if macros are used or written in any particular way.
